...
def predict_engine(ticker, interval, period, future_steps, offset=0):
    ...
    try:
        ...
        # 評価対象（過去または現在）の最新1行を取得
        target_row = eval_df.dropna().tail(1)
        last_price = float(target_row['Close'].iloc[0])
        last_rsi = float(target_row['RSI'].iloc[0])
        pred = model.predict(target_row[features])[0]
        prob = model.predict_proba(target_row[features])[0]
...
if current_price == 0:
    st.error("⚠️ レート取得エラー：市場閉場中か通信制限の可能性があります。")
else:
    st.markdown(f"""
...
timeframes = {
    ...
    "1日": {"p": ("1mo","1d",1), "o": 1} # 1日前を確実に取るために1ヶ月分取得
}
