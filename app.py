...
def predict_engine(ticker, interval, period, future_steps, offset=0):
    try:
        ...
        if isinstance(df['Close'], pd.DataFrame):
            for col in ['Open', 'High', 'Low', 'Close']: df[col] = df[col].iloc[:, 0]
        ...
        # モデル強化: 予測のバラツキを抑え、安定性を向上
        model = RandomForestClassifier(n_estimators=300, max_depth=12, min_samples_leaf=5, random_state=42)
...
st.caption(f"精度強化モデル稼働中 | 更新: {jst_now.strftime('%H:%M')}")
...
if st.button('🔄 全時間軸を再学習して更新'): st.rerun()
...
for i, (label, cfg) in enumerate(timeframes.items()):
    with cols[i]:
        ...
        p_val, p_dir, _, p_rsi = predict_engine("JPY=X", *cfg["p"], offset=cfg["o"])
        ...
        _, f_dir, f_prob, f_rsi = predict_engine("JPY=X", *cfg["p"], offset=0)
        
        # 判定の厳格化（確信度55%未満は中立とする）
        if max(f_prob) < 0.55:
            display_text = "⚖️中立"
            delta_val = "迷い"
        else:
            display_text = "📈上昇" if f_dir == 1 else "📉下落"
            delta_val = f"{max(f_prob)*100:.1f}%"
            
        st.metric("", display_text, delta_val)
        st.markdown(f'<p class="tech-subtext">RSI: {f_rsi:.1f}</p>', unsafe_allow_html=True)

st.divider()
st.subheader("📅 経済指標")
st.link_button("🌐 GMO外貨 カレンダー", "https://www.gaikaex.com/gaikaex/mark/calendar/", use_container_width=True)
